...
def plot_learning_curve(estimator, title, X, y, ylim=None, cv=None,
    ...
    train_sizes, train_scores, test_scores, fit_times, _ = learning_curve(
        clf, X, y, cv=cv, n_jobs=n_jobs, train_sizes=train_sizes,scoring=scoring, return_times=True)
...
def plot_learning_curve_save(estimator, title, X, y, ylim=None, cv=None,
                        n_jobs=None,scoring=None, train_sizes=np.linspace(.1, 1.0, 5)):
    """
    Generate a simple plot of the test and training learning curve.
    ...
    if scoring == "neg_root_mean_squared_error" or "neg_mean_absolute_error":
        train_scores_mean = -np.mean(train_scores, axis=1)
        train_scores_std = -np.std(train_scores, axis=1)
        test_scores_mean = -np.mean(test_scores, axis=1)
        test_scores_std = -np.std(test_scores, axis=1)
    ...
    a=a.replace("\n","")
    a="".join(a.split())
    plt.savefig(a+"_"+scoring+".pdf",format="pdf",bbox_inches="tight")
    return plt

def plot_fit_times_curve(estimator, title, X, y, ylim=None, cv=None,
                        n_jobs=None,scoring=None, train_sizes=np.linspace(.1, 1.0, 5)):
    """
    Generate a simple plot of the test and training learning curve.
    ...
    plt.plot(train_sizes, fit_times_mean, 'o-', color="b",
             label="Scalability of model")

    a=str(estimator)
    a=a.replace("\n","")
    a="".join(a.split())
    plt.savefig(a+"_"+"scalability.pdf",format="pdf",bbox_inches="tight")
    return plt